...
Describe "Utils test - docker nat and winnat disabled on startup" {
    ...
    BeforeAll {
        ...
        Write-Log "before install:"
        Write-Log "stopping docker"
        Invoke-Command -Session $Session {
            Stop-Service Docker
        }
        Write-Log "creating conf dir"
        Invoke-Command -Session $Session {
            New-Item -Type Directory -Force 'C:\ProgramData\Docker\config'
        }
        Write-Log "creating conf file"
        Invoke-Command -Session $Session {
            '"bridge" : "none"' | Set-Content -Path 'C:\ProgramData\Docker\config\daemon.json' -Force
        }
        Write-Log "starting docker again"
        Invoke-Command -Session $Session {
