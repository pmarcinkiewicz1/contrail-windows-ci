# Script based on Samuel Colvin's dnserver.py
...
class Resolver(ProxyResolver):
    def __init__(self, zone_file):
        super(Resolver, self).__init__('0.0.0.0', 53, 5)
    ...
    def load_zones(self, zone_file):
        assert zone_file.exists(), 'zone files "%s" does not exist' % zone_file
        logger.info('loading zone file "%s":', zone_file)
        zones = []
        for line in self.zone_lines():
            try:
                rname, rtype, args_ = line.split()
                ...
                raise RuntimeError('Error processing line (%s: %s) "%s"' % (e.__class__.__name__, e, line.strip()))
...
if __name__ == '__main__':
    ...
    zone_file = Path(os.getenv('ZONE_FILE', 'C:/zones/zones'))
